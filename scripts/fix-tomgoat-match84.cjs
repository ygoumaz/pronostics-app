/**
 * Fix script: update TomGoat's pronostic for match 84 (Spain vs Austria)
 * to 2-0 (Spain wins). Inserts the pronostic if it doesn't exist yet.
 *
 * Run from /app on the Fly.io machine so node_modules resolves correctly:
 *   node /app/fix-tomgoat-match84.cjs
 */
...
const MATCH_NUMBER = 84;
const EXPECTED_HOME_CODE = 'ESP'; // Spain
const EXPECTED_AWAY_CODE = 'AUT'; // Austria
const HOME_GOALS = 2;
const AWAY_GOALS = 0;

async function main() {
  // --- Find participant "TomGoat" (case-insensitive on both email and displayName) ---
